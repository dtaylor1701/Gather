...
struct Gather: ParsableCommand {
    ...
    @Flag(name: .customLong("gemini"), help: "Generate a GEMINI.md file with imports instead of inline content.")
    ...
    private func analyzeContext(at path: String) -> String {
        ...
        if fm.fileExists(atPath: rootURL.appendingPathComponent("Package.swift").path) {
            ...
            if let packageContent = try? String(contentsOf: rootURL.appendingPathComponent("Package.swift"), encoding: .utf8) {
                let patterns = [
                ...
                for pattern in patterns {
                    ...
                    let nsRange = NSRange(packageContent.startIndex..<packageContent.endIndex, in: packageContent)
                    regex?.enumerateMatches(in: packageContent, options: [], range: nsRange) { match, _, _ in
                        if let match = match, let range = Range(match.range(at: match.numberOfRanges - 1), in: packageContent) {
        ...
        } else if contents.contains(where: { $0.hasSuffix(".xcodeproj") || $0.hasSuffix(".xcworkspace") }) {
        ...
        if !targets.isEmpty { contextInfo += "- **Targets/Products**: \(targets.joined(separator: ", "))\n" }
        
        let ignoredFolders = [".git", ".build", ".swiftpm", "node_modules", ".xcodeproj", ".xcworkspace"]
        let topDirs = contents.filter { item in
            var isDir: ObjCBool = false
            return fm.fileExists(atPath: rootURL.appendingPathComponent(item).path, isDirectory: &isDir) && 
                   isDir.boolValue && 
                   !ignoredFolders.contains(where: { item.hasSuffix($0) })
        }
        
        if !topDirs.isEmpty { contextInfo += "- **Top-level Directories**: \(topDirs.joined(separator: ", "))\n" }
        contextInfo += "\n"
        return contextInfo
    }
}
